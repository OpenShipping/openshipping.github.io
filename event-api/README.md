...
The above diagram provides a simple example of the heterogeneous groups of data providers passing events to a data consumer, and how that data consumer interprets the events through applying business logic on the data stream or persisted. 

Interpreted event streams can be subsequently exposed as value added services. Particularly, the [Tracking API](https://github.com/OpenShipping/openshipping.github.io/tree/master/tracking-api) is meant to serve this purpose, and is based on the same common logical data model as this Event API.
